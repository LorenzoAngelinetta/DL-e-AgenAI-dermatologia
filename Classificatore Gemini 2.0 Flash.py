...
istruzioni = [
    "Analyze the image of the skin lesion and classify it into ONLY ONE of the following class.",

    "Class Descriptions:",
    "- Melanocytic nevi (nv): are round, smooth, and uniform in color, typically appearing in shades of brown, black, or pink. They are well-defined, symmetrical, and do not change significantly in size, shape, or color over time. The surface is usually flat or slightly raised, with a consistent and even texture. The borders are sharp and clear, with no irregularities. Nevi are typically small, with a diameter 4-5mm, and remain stable in size throughout life. However, SOME nevi may exhibit mild asymmetry or slight color variation, which could lead to MISCLASSIFICATION as melanoma. Large congenital nevi or those with uneven edges may be particularly challenging for automated models to differentiate. The surrounding skin is typically unaffected, without visible redness or swelling"
    ...
    "- Dermatofibromas (df): are firm, small, brown (brownish) nodules that exhibit a central dimpling effect when pressed. These lesions are round and well-defined with smooth or slightly raised surfaces. Dermatofibromas are usually harmless and do not require treatment unless they become painful, irritated, or cosmetically concerning. The dimpling effect upon pressure is a distinguishing feature of this type of lesion.",
    ...
    "- Melanoma (mel): lesions are dark, irregularly shaped, and asymmetrical, with jagged or notched borders. They exhibit multiple shades of dark, red, blue, or white. Melanomas are usually larger than 6mm in diameter. These lesions often have a rough or scaly texture, and the surface may be ulcerated or raised unevenly. The borders are blurry or undefined, often showing irregularities. The ABCDE rule (Asymmetry, Border irregularity, Color variation, Diameter larger than 6mm, Evolution over time) is often used to identify melanoma. Additionally, the surrounding skin may show signs of inflammation, redness, or even bleeding, indicating a more aggressive or advanced stage.",

    "Your response must include ONLY ONE of the following classes (the most likely class).",
    "When analyzing the skin lesion, carefully consider its shape, edges, surface (smooth or irregular), color, symmetry. These details are crucial for determining the correct category of the lesion."

    "Follow these steps for classification:",
    "1: Analyze the deteails and features of the lesion and identify features.",
    "2: Compare the image with the descriptions and determine the best matching class.",
    "3: Your response must include ONLY ONE of the following classes (the most likely class among mel, nv, akiec, bcc, bkl, df,  vasc)."
]
...
class Agent_Gemini:
    ...
    def esegui(self, testo, immagine):
        img = Image(filepath=immagine)
        risposta_gemini = self.agent.run(testo, images=[img]).content #risposta ottenuta da Gemini
    ...
    def salvataggio_analisi(self, path_valutazione, testo):
        ...
        for img_path in img_test:
            ...
            self.dati["valutate"].append({
                ...
                "classificazione_gemini": classificazione_gemini,
                "classificazione_gemini_numerica": classificazione_gemini_numerica
            })
            ...
            print(f"Image: {img_path} - Real: {classificazione_reale} - Gemini prediction: {classificazione_gemini}")
    ...
    def calcola_metriche(self):
        ...
        predetto = [d["classificazione_gemini_numerica"] for d in self.dati["valutate"]]
        ...
        print("Confusion Matrix Classification Gemini:\n")
        ...
        plt.ylabel("Real")
        plt.title("Confusion Matrix - Classification Skin Lesion Gemini")
...
input_testuale = "Classify this lesion into ONLY ONE of the following class: akiec, bcc, bkl, df, mel, nv, vasc (use the characteristics of the description to find the most likely class and paying close attention not to confuse nv with mel)."
